...
-- Add bev_package column
ALTER TABLE public.roster_day_events
  ADD COLUMN IF NOT EXISTS bev_package text DEFAULT NULL;
...
CREATE OR REPLACE FUNCTION public.get_employee_day_events(_employee_code text, _business_code text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, date date, event_space text, event_type text, num_tables integer, chairs_per_table integer, tablecloth_color text, cold_sparkles boolean, dry_ice boolean, red_carpet boolean, decor_access boolean, notes text, adult_guests integer, kids_guests integer, smoke_machine boolean, live_stall boolean, live_stall_details text, host_name text, host_contact_number text, bev_package text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF is_supervisor_or_manager THEN
    ...
    SELECT rde.id, rde.date, rde.event_space, rde.event_type,
      ...
      rde.host_name, rde.host_contact_number, rde.bev_package
