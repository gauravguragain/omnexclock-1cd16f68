...
CREATE OR REPLACE FUNCTION public.get_employee_day_events(
  _employee_code text,
  _business_code text DEFAULT NULL
)
RETURNS TABLE(
  id uuid,
  date date,
  ...
  live_stall boolean,
  live_stall_details text,
  host_name text,
  host_contact_number text,
  bev_package text,
  event_time text,
  runsheet_url text
)
