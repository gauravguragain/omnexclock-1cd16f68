...
-- Session-based timesheet builder: groups clock_in -> clock_out spans into
-- a single row pinned to the clock-in date, so overnight shifts no longer
-- split across two days. Also exposes a crossed_midnight flag.

-- Drop both overloads, then recreate with crossed_midnight column
DROP FUNCTION IF EXISTS public.get_employee_timesheets(text);
DROP FUNCTION IF EXISTS public.get_employee_timesheets(text, text);
...
BEGIN
  ...
  -- Build sessions in a temp table
  ...
  FOR ev IN
    SELECT ce.event_type, ce.timestamp AS ev_time,
  ...
  LOOP
    ...
    ELSIF ev.event_type = 'clock_out' THEN
      IF current_open_idx IS NOT NULL THEN
        ...
        SET clock_out_t = ev.ev_time,
            -- close any dangling break that crossed
      ...
      ELSE
        ...
        session_idx := session_idx + 1;
        INSERT INTO _ts_sessions(idx, s_date, clock_out_t, break_mins, is_open)
        VALUES (session_idx, ev.ev_date, ev.ev_time, 0, false);
      END IF;
    ...
    ELSIF ev.event_type = 'break_start' THEN
      ...
      ELSE
        session_idx := session_idx + 1;
        INSERT INTO _ts_sessions(idx, s_date, current_break_start, first_break_start, break_mins, is_open)
        VALUES (session_idx, ev.ev_date, ev.ev_time, ev.ev_time, 0, false);
      END IF;
    ...
    ELSIF ev.event_type = 'break_end' THEN
      IF current_open_idx IS NOT NULL THEN
        ...
        WHERE s.idx = current_open_idx;
      ELSE
        -- attach to most recent open-break orphan on this date, else create
        UPDATE _ts_sessions s
        SET break_mins = s.break_mins + COALESCE(
              EXTRACT(EPOCH FROM (ev.ev_time - s.current_break_start)) / 60, 0
            ),
            ...
            current_break_start = NULL
        WHERE s.idx = (
          SELECT idx FROM _ts_sessions
          WHERE current_break_start IS NOT NULL AND s_date = ev.ev_date
          ORDER BY idx DESC LIMIT 1
        );
        IF NOT FOUND THEN
          session_idx := session_idx + 1;
          INSERT INTO _ts_sessions(idx, s_date, last_break_end, break_mins, is_open)
          VALUES (session_idx, ev.ev_date, ev.ev_time, 0, false);
        END IF;
      END IF;
    END IF;
...
-- Keep single-arg overload as a thin wrapper
