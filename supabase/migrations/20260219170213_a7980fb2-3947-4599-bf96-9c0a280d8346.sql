...
ALTER TABLE public.roster_day_events ADD COLUMN IF NOT EXISTS event_time text DEFAULT NULL;

-- Recreate function with event_time in return
DROP FUNCTION IF EXISTS public.get_employee_day_events(text);
...
RETURNS TABLE(
  ...
  num_tables integer, chairs_per_table integer, tablecloth_color text,
  cold_sparkles boolean, dry_ice boolean, red_carpet boolean,
  decor_access boolean, notes text, adult_guests integer, kids_guests integer,
  smoke_machine boolean, live_stall boolean, live_stall_details text,
  host_name text, host_contact_number text, bev_package text, event_time text
)
