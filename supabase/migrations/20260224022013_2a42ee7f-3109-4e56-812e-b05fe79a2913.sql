...
-- Fix the non-business-code overload to also handle overnight shifts
...
BEGIN
  ...
  shift_dates AS (
    ...
    FROM shift_assigned sa WHERE sa.shift_num > 0
  ),
  aggregated AS (
    SELECT sd.shift_date AS ev_date,
    ...
    FROM shift_dates sd GROUP BY sd.shift_date
  )
