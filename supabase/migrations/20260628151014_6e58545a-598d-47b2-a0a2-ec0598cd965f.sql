...
BEGIN
  ...
  LOOP
    ...
    ELSIF ev.event_type = 'clock_out' THEN
      ...
      ELSE
        -- Only create a standalone clock-out row when no prior clock-in session exists.
        -- This prevents post-midnight clock-outs from becoming next-day timesheets.
        IF NOT EXISTS (
          SELECT 1 FROM _ts_sessions s
          WHERE s.clock_in_t IS NOT NULL
            AND s.clock_out_t IS NULL
            AND ev.ev_time > s.clock_in_t
        ) THEN
          session_idx := session_idx + 1;
