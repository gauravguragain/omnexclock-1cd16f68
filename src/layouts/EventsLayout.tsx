...
export default function EventsLayout() {
  ...
  return <div className="min-h-dvh bg-background lg:flex">
    <aside className="hidden w-64 shrink-0 border-r border-border/40 lg:block"><div className="sticky top-0 h-dvh overflow-y-auto">
    ...
    <header className="sticky top-0 z-30 flex items-center justify-between border-b border-border/40 bg-background/95 p-3 backdrop-blur lg:hidden"><p className="font-semibold">Events & Sales</p><Button size="icon" variant="ghost" onClick={() => setOpen(!open)}>{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</Button></header>
    {open && <div className="fixed inset-0 top-14 z-20 overflow-y-auto bg-background lg:hidden">{nav}<div className="p-4"><Link to={`/b/${businessCode}/admin`} className="text-sm text-muted-foreground">Business Admin</Link></div></div>}
