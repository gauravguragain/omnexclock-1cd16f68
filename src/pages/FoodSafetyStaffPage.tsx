    ...
    <div className="pt-10 text-center">
      ...
      <div className="grid grid-cols-3 gap-3">
        {["1", "2", "3", "4", "5", "6", "7", "8", "9", "", "0", "⌫"].map((k) => k === "" ? <div key="x" /> : (
          <button key={k} onClick={() => setPin((p) => (k === "⌫" ? p.slice(0, -1) : (p + k).slice(0, 8)))} className="h-16 rounded-2xl border border-zinc-300 bg-zinc-100 text-zinc-950 dark:border-zinc-300 dark:bg-zinc-100 dark:text-zinc-950 text-2xl font-semibold active:scale-95 flex items-center justify-center">{k === "⌫" ? <Delete className="h-6 w-6" /> : k}</button>
        ))}
      </div>
