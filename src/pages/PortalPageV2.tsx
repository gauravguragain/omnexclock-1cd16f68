...
function EventCard({ ev }: { ev: any }) {
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-secondary/30 overflow-hidden">
      <button className="w-full px-3 py-2.5 text-left flex items-center gap-2 hover:bg-secondary/50 transition-colors" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-center gap-1.5 flex-1 min-w-0 overflow-hidden">
          {ev.event_type && <Badge variant="outline" className="text-[10px] shrink-0 max-w-[80px] truncate">{ev.event_type}</Badge>}
          {ev.host_name ? <span className="text-xs font-medium text-foreground truncate">{ev.host_name}</span>
            : !ev.event_type ? <span className="text-xs text-muted-foreground italic">Event</span> : null}
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          {ev.event_time && <span className="text-[10px] text-muted-foreground flex items-center gap-0.5 whitespace-nowrap"><Clock className="h-3 w-3" /> {ev.event_time}</span>}
          {totalGuests > 0 && <span className="text-[10px] text-muted-foreground whitespace-nowrap"><Users className="h-3 w-3 inline" /> {totalGuests}</span>}
      ...
      {expanded && (
        <div className="px-3 pb-3 pt-1 border-t border-border/30 space-y-2.5">
          ...
          {ev.runsheet_url && (
            <button onClick={(e) => { e.stopPropagation(); viewPdf(ev.runsheet_url); }} className="flex items-center gap-1.5 text-xs text-primary hover:underline mt-1">
...
export default function PortalPageV2() {
  ...
  useEffect(() => {
    ...
    const loadBusiness = async () => {
      ...
      if (data) {
        ...
        if (data.theme) applyTheme(data.theme as any);
      }
    };
    loadBusiness();
    return () => { resetTheme(); };
  ...
  const handleLogin = async () => {
    ...
    await runAction(async () => {
      const ok = await portal.login(code, bizCode);
      if (!ok) {
  ...
  const openTsHistory = async (workDate: string) => {
    ...
    try {
      const { data } = await supabase.rpc("get_employee_timesheet_history", {
        _employee_code: portal.employeeCode,
        _date: workDate,
  ...
  const loadForumPost = async (post: any) => {
    ...
    const [commentsRes, reactionsRes] = await Promise.all([
      supabase.rpc("get_forum_comments", { _employee_code: portal.employeeCode, _post_id: post.id, _business_code: bizCode }),
      supabase.rpc("get_my_reactions", { _employee_code: portal.employeeCode, _post_id: post.id, _business_code: bizCode }),
    ]);
  ...
  const sendComment = async () => {
    ...
    await runAction(async () => {
      await supabase.rpc("add_forum_comment", { _employee_code: portal.employeeCode, _post_id: selectedForumPost.id, _content: newComment.trim(), _business_code: bizCode });
      setNewComment("");
  ...
  const toggleReaction = async (emoji: string) => {
    ...
    await supabase.rpc("toggle_forum_reaction", { _employee_code: portal.employeeCode, _post_id: selectedForumPost.id, _reaction: emoji, _business_code: bizCode });
    const [reactionsRes] = await Promise.all([
      supabase.rpc("get_my_reactions", { _employee_code: portal.employeeCode, _post_id: selectedForumPost.id, _business_code: bizCode }),
      portal.refreshData(),
    ]);
    setMyReactions(new Set((reactionsRes.data || []).map((r: any) => r.reaction)));
    // Update selected post with refreshed forum data
    const updated = portal.forumPosts.find((p: any) => p.id === selectedForumPost.id);
  ...
  const resetRequestForm = () => {
    setReqType("leave"); setReqStartDate(undefined); setReqEndDate(undefined);
    setReqIsRecurring(false); setReqDays([]); setReqRecurringStart(undefined);
    setReqRecurringEnd(undefined); setReqReason(""); setReqStartTime(""); setReqEndTime("");
  ...
  const submitRequest = async () => {
    await runAction(async () => {
      setReqSaving(true);
      const params: any = {
        _employee_code: portal.employeeCode, _request_type: reqType, _business_code: bizCode,
        _is_recurring: reqIsRecurring, _reason: reqReason.trim() || null,
      ...
      if (editingRequestId) {
        const { data } = await supabase.rpc("update_employee_request", { ...params, _request_id: editingRequestId });
        if (data) { toast({ title: "Request Updated" }); setRequestOpen(false); resetRequestForm(); await portal.refreshData(); }
        else toast({ title: "Error", description: "Failed to update request.", variant: "destructive" });
      } else {
        const { data } = await supabase.rpc("submit_employee_request", params);
        if (data) { toast({ title: "Request Submitted", description: "Your request has been sent for admin approval." }); setRequestOpen(false); resetRequestForm(); await portal.refreshData(); }
        else toast({ title: "Error", description: "Failed to submit request.", variant: "destructive" });
      }
      setReqSaving(false);
    });
  };
  ...
  const deleteRequest = async (requestId: string) => {
    await runAction(async () => {
      const { data } = await supabase.rpc("delete_employee_request", { _employee_code: portal.employeeCode, _request_id: requestId, _business_code: bizCode });
      if (data) { toast({ title: "Request deleted" }); await portal.refreshData(); }
      else toast({ title: "Error", description: "Failed to delete request.", variant: "destructive" });
    });
  };
  ...
  const openEditRequest = (req: any) => {
    ...
    if (req.is_recurring) {
      ...
      setReqStartDate(undefined); setReqEndDate(undefined);
    } else {
      ...
      setReqDays([]); setReqRecurringStart(undefined); setReqRecurringEnd(undefined);
    }
    setRequestOpen(true);
  };
  ...
  const shiftsByWeek = useMemo(() => {
    const todayMonday = getMondayStr(ausToday());
    const weeks: Record<string, PortalShift[]> = {};
    for (const s of portal.shifts) {
      if (s.week_start_date < todayMonday) continue;
      if (!weeks[s.week_start_date]) weeks[s.week_start_date] = [];
      weeks[s.week_start_date].push(s);
    }
  ...
  const thisWeekHours = useMemo(() => {
    const todayMonday = getMondayStr(ausToday());
    return portal.shifts
      .filter(s => s.week_start_date === todayMonday)
      .reduce((sum, s) => sum + (s.hours_worked ?? calcNetHours(s.start_time, s.end_time, s.break_minutes)), 0);
  }, [portal.shifts]);

  const lastSyncLabel = portal.lastSyncedAt
    ? `Synced ${portal.lastSyncedAt.toLocaleTimeString("en-AU", { hour: "2-digit", minute: "2-digit" })}`
    : null;
      ...
      <div className="min-h-[100dvh] bg-background flex flex-col items-center justify-center p-4 standalone-top-pad overflow-y-auto">
        ...
        <Card className="w-full max-w-sm gold-border border gold-glow">
          <CardContent className="p-6 space-y-4">
            ...
            <Input value={code} readOnly className="text-center text-3xl tracking-[0.5em] font-mono h-16 bg-surface" placeholder="••••" />
            <div className="grid grid-cols-3 gap-2">
              {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map(n => (
                <Button key={n} variant="secondary" className="h-14 sm:h-16 text-2xl font-bold touch-active btn-press select-none" onClick={() => handleNumpadClick(n)}>{n}</Button>
              ))}
              <Button variant="secondary" className="h-14 sm:h-16 touch-active btn-press select-none" onClick={() => setCode("")}><ArrowLeft className="h-6 w-6" /></Button>
              <Button variant="secondary" className="h-14 sm:h-16 text-2xl font-bold touch-active btn-press select-none" onClick={() => handleNumpadClick("0")}>0</Button>
              <Button variant="secondary" className="h-14 sm:h-16 touch-active btn-press select-none" onClick={() => setCode(p => p.slice(0, -1))}><Delete className="h-6 w-6" /></Button>
            </div>
            <Button className="w-full h-14 text-lg touch-active btn-press" onClick={handleLogin} disabled={code.length !== 4 || portal.loading}>
              {portal.loading ? "Loading..." : "View My Portal"}
            </Button>
          </CardContent>
        </Card>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <button className="flex items-center gap-2 hover:opacity-80 transition-opacity" onClick={() => setAccessInfoOpen(true)}>
                ...
                <span className="font-semibold text-foreground">{portal.employeeInfo?.employee_name}</span>
                <Download className="h-3 w-3 text-muted-foreground" />
              </button>
              <div className="flex items-center gap-2 mt-0.5">
                <Badge variant="outline" className="text-[10px] px-1.5 py-0 font-mono">{portal.employeeCode}</Badge>
                <Badge className={`text-[10px] px-1.5 py-0 ${
                  portal.employeeInfo?.current_status === "clocked_in" ? "bg-success/15 text-success" :
                  portal.employeeInfo?.current_status === "on_break" ? "bg-warning/15 text-warning" :
                  "bg-muted text-muted-foreground"
                }`}>
                  {portal.employeeInfo?.current_status === "clocked_in" ? "Clocked In" :
                   portal.employeeInfo?.current_status === "on_break" ? "On Break" : "Clocked Out"}
                </Badge>
                {lastSyncLabel && (
                  <span className="text-[9px] text-muted-foreground/60">{lastSyncLabel}</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="max-w-3xl mx-auto p-4 space-y-4 pb-8">
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                {(() => {
                  ...
                  const next = portal.shifts.find(s => s.date >= today);
          ...
          <Tabs defaultValue="today" className="w-full">
            ...
            <TabsContent value="today" className="space-y-4 mt-4">
              <TodayTab employeeCode={portal.employeeCode} businessCode={bizCode} shifts={portal.shifts} />
            </TabsContent>
            ...
            <TabsContent value="roster" className="space-y-4 mt-4">
              {shiftsByWeek.length === 0 ? (
                ...
                shiftsByWeek.map(([weekStart, weekShifts]) => {
                  ...
                  const weekTotal = weekShifts.reduce((sum, s) => sum + (s.hours_worked ?? calcNetHours(s.start_time, s.end_time, s.break_minutes)), 0);
                  return (
                    <Card key={weekStart}>
                      <CardHeader className="pb-2 px-4 pt-4">
            ...
            <TabsContent value="forum" className="space-y-3 mt-4">
              {portal.forumPosts.length === 0 ? (
                ...
                portal.forumPosts.map((post: any) => (
                  <Card key={post.id} className="cursor-pointer hover:border-primary/30 transition-colors" onClick={() => loadForumPost(post)}>
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center gap-2 mt-2 flex-wrap">
                        <span className="text-[10px] text-muted-foreground">{toAusLocaleString(new Date(post.created_at), { day: "numeric", month: "short" })}</span>
                        <Badge variant="secondary" className="text-[10px] px-1.5 py-0"><MessageSquare className="h-2.5 w-2.5 mr-0.5" /> {post.comment_count}</Badge>
                        {post.reaction_counts && Object.entries(post.reaction_counts as Record<string, number>).map(([emoji, count]) => (
                          <Badge key={emoji} variant="secondary" className="text-[10px] px-1.5 py-0">{emoji} {count}</Badge>
                        ))}
                      </div>
                    </CardContent>
                  </Card>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => {
                            if (window.confirm("Are you sure you want to delete this request?")) deleteRequest(req.id);
                          }}>
          ...
          <Dialog open={!!selectedForumPost} onOpenChange={(open) => { if (!open) setSelectedForumPost(null); }}>
            <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
              ...
              <div className="flex gap-2 flex-wrap">
                {["👍", "❤️", "😂", "🎉"].map(emoji => (
                  <Button key={emoji} variant={myReactions.has(emoji) ? "default" : "outline"} size="sm" className="h-8 text-base" onClick={() => toggleReaction(emoji)}>
              ...
              <div className="space-y-2">
                  ...
                  forumComments.map((c: any) => (
                    <div key={c.id} className="rounded-lg bg-secondary/50 px-3 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-muted-foreground">{toAusLocaleString(new Date(c.created_at), { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</span>
                      </div>
                ...
                <div className="flex gap-2">
                  <Input placeholder="Write a comment..." value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => e.key === "Enter" && sendComment()} className="flex-1" />
                  <Button size="sm" onClick={sendComment} disabled={!newComment.trim()}><Send className="h-4 w-4" /></Button>
                </div>
              </div>
            </DialogContent>
          </Dialog>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {DAYS_OF_WEEK.map(day => (
                          <Button key={day} variant={reqDays.includes(day) ? "default" : "outline"} size="sm" className="text-xs h-7"
                            onClick={() => setReqDays(prev => prev.includes(day) ? prev.filter(d => d !== day) : [...prev, day])}>{day.slice(0, 3)}</Button>
                        ))}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !reqRecurringStart && "text-muted-foreground")}>
                              <CalendarIcon className="mr-2 h-4 w-4" />{reqRecurringStart ? format(reqRecurringStart, "dd MMM yyyy") : "Pick date"}
                            </Button>
                          </PopoverTrigger>
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !reqRecurringEnd && "text-muted-foreground")}>
                              <CalendarIcon className="mr-2 h-4 w-4" />{reqRecurringEnd ? format(reqRecurringEnd, "dd MMM yyyy") : "Pick date"}
                            </Button>
                          </PopoverTrigger>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !reqStartDate && "text-muted-foreground")}>
                            <CalendarIcon className="mr-2 h-4 w-4" />{reqStartDate ? format(reqStartDate, "dd MMM yyyy") : "Pick date"}
                          </Button>
                        </PopoverTrigger>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !reqEndDate && "text-muted-foreground")}>
                            <CalendarIcon className="mr-2 h-4 w-4" />{reqEndDate ? format(reqEndDate, "dd MMM yyyy") : "Same as start"}
                          </Button>
                        </PopoverTrigger>
            ...
            <div className="space-y-3">
              {tsHistoryLogs.map((log: any, i: number) => {
                ...
                const actionLabels: Record<string, string> = {
                  timesheet_edit: "Edited", timesheet_add: "Added", timesheet_delete: "Deleted",
                  timesheet_approve: "Approved", timesheet_unapprove: "Approval Revoked",
                };
                return (
                  <div key={i} className="border border-border rounded-lg p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <Badge variant="outline" className="text-xs">{actionLabels[log.log_action] || log.log_action}</Badge>
                      <span className="text-xs text-muted-foreground">{new Date(log.log_timestamp).toLocaleString("en-AU", { dateStyle: "medium", timeStyle: "short" })}</span>
                    </div>
                    {d?.comment && <p className="text-sm text-foreground"><span className="text-muted-foreground">Comment:</span> {d.comment}</p>}
      ...
      <Dialog open={accessInfoOpen} onOpenChange={setAccessInfoOpen}>
        <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedDevice} onValueChange={setSelectedDevice}>
                ...
                <SelectContent className="bg-card border border-border z-[200]">
                  <SelectItem value="iphone"><div className="flex items-center gap-2"><Smartphone className="h-4 w-4" /> iPhone (Safari)</div></SelectItem>
                  <SelectItem value="android"><div className="flex items-center gap-2"><Smartphone className="h-4 w-4" /> Android (Chrome)</div></SelectItem>
                  <SelectItem value="ipad"><div className="flex items-center gap-2"><Tablet className="h-4 w-4" /> iPad (Safari)</div></SelectItem>
                  <SelectItem value="desktop"><div className="flex items-center gap-2"><Monitor className="h-4 w-4" /> Desktop (Chrome / Edge)</div></SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <p className="text-[10px] text-muted-foreground/70">💡 Your employee code is confidential. Do not share it with others.</p>
          </div>
        </DialogContent>
      </Dialog>
