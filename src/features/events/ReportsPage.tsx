...
export default function ReportsPage() {
  ...
  const r = useMemo(() => {
    ...
    const bookings = crm.bookings.filter(b => b.status !== "cancelled" && b.event_date >= from);
    const byMonth = Array.from({ length: months }, (_, i) => { const m = format(subMonths(new Date(), months - 1 - i), "yyyy-MM"); const list = bookings.filter(b => String(b.event_date).startsWith(m)); return { month: format(new Date(`${m}-01T00:00:00`), "MMM yy"), events: list.filter(b => (b.booking_kind || "event") === "event").length, catering: list.filter(b => b.booking_kind === "catering").length, revenue: list.reduce((s, b) => s + Number(b.total_amount || 0), 0) }; });
    ...
    const byHall = group(b => (b.booking_kind === "catering" ? "Off-site catering" : b.venue_space));
    const leads = crm.leads.filter(l => l.created_at >= from);
  ...
  const tables = [
    { title: "Events per month", head: ["Month", "Events", "Catering", "Revenue"], rows: r.byMonth.map(m => [m.month, m.events, m.catering, money(m.revenue)]) },
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Events vs catering per month</CardTitle></CardHeader><CardContent className="h-72"><ResponsiveContainer><BarChart data={r.byMonth}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="month" fontSize={11} stroke="hsl(var(--muted-foreground))" /><YAxis allowDecimals={false} fontSize={11} stroke="hsl(var(--muted-foreground))" /><Tooltip contentStyle={{ background: "hsl(var(--popover))", border: "1px solid hsl(var(--border))" }} /><Bar dataKey="events" stackId="a" fill="hsl(var(--primary))" /><Bar dataKey="catering" stackId="a" fill="hsl(var(--muted-foreground))" /></BarChart></ResponsiveContainer></CardContent></Card>
