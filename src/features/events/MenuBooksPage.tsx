...
type CourseDraft = { key: string; name: string; picks: string; vegPicks: string; nonVegPicks: string; items: string[]; proteins?: Record<string, string> };
...
function PackageEditor({ open, onClose, pkg, data: d }: { open: boolean; onClose: () => void; pkg: Row | null; data: ReturnType<typeof useEventsData> }) {
  const initialCourses: CourseDraft[] = pkg ? d.courses.filter(c => c.package_id === pkg.id).map(c => ({ key: c.id, name: c.name, picks: c.picks != null ? String(c.picks) : "", vegPicks: c.veg_picks != null ? String(c.veg_picks) : "", nonVegPicks: c.non_veg_picks != null ? String(c.non_veg_picks) : "", items: d.courseItems.filter(ci => ci.course_id === c.id).map(ci => ci.dish_id || `drink:${ci.drink_id}`), proteins: Object.fromEntries(d.courseItems.filter(ci => ci.course_id === c.id && ci.dish_id && ci.protein_options?.length).map(ci => [ci.dish_id, (ci.protein_options || []).join(", ")])) })) : [{ key: crypto.randomUUID(), name: "Starters", picks: "", vegPicks: "", nonVegPicks: "", items: [] }];
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const [i, c] of courses.entries()) {
        ...
        if (c.items.length) { const ri = await (supabase.from("crm_package_course_items" as any) as any).insert(c.items.map(it => it.startsWith("drink:") ? { business_id: bid, course_id: r.data.id, drink_id: it.slice(6) } : { business_id: bid, course_id: r.data.id, dish_id: it, protein_options: (c.proteins?.[it] || "").split(",").map(x => x.trim()).filter(Boolean) })); if (ri.error) throw ri.error; }
      }
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_260px]">
      <div className="space-y-4">
        ...
        {courses.map((c, i) => <div key={c.key} className="space-y-3 rounded-md border p-4">
          ...
          <div className="space-y-1.5">{c.items.map(it => { const p = pool.find(x => x.id === it); return <div key={it} className="flex flex-wrap items-center gap-2"><Badge variant="outline" className="gap-1">{p?.tag && <span className="text-primary">{p.tag}</span>}{p?.name || "Archived item"}<button onClick={() => upd(c.key, { items: c.items.filter(x => x !== it) })}><X className="h-3 w-3" /></button></Badge>{!bev && <Input className="h-7 min-w-40 flex-1 text-xs" placeholder="Protein choices (optional), e.g. Chicken, Lamb, Goat" value={c.proteins?.[it] || ""} onChange={e => upd(c.key, { proteins: { ...(c.proteins || {}), [it]: e.target.value } })} />}</div>; })}</div>
