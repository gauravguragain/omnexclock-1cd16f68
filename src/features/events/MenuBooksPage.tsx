...
type CourseDraft = { key: string; name: string; picks: string; items: string[]; proteins?: Record<string, string> };
...
function PackageEditor({ open, onClose, pkg, data: d }: { open: boolean; onClose: () => void; pkg: Row | null; data: ReturnType<typeof useEventsData> }) {
  const initialCourses: CourseDraft[] = pkg ? d.courses.filter(c => c.package_id === pkg.id).map(c => ({ key: c.id, name: c.name, picks: c.picks ? String(c.picks) : "", items: d.courseItems.filter(ci => ci.course_id === c.id).map(ci => ci.dish_id || `drink:${ci.drink_id}`), proteins: Object.fromEntries(d.courseItems.filter(ci => ci.course_id === c.id && ci.dish_id && ci.protein_options?.length).map(ci => [ci.dish_id, (ci.protein_options || []).join(", ")])) })) : [{ key: crypto.randomUUID(), name: "Starters", picks: "", items: [] }];
