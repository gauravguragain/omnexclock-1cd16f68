...
function PackageEditor({ open, onClose, pkg, data: d }: { open: boolean; onClose: () => void; pkg: Row | null; data: ReturnType<typeof useEventsData> }) {
  const initialCourses: CourseDraft[] = pkg ? d.courses.filter(c => c.package_id === pkg.id).map(c => ({ key: c.id, name: c.name, picks: c.picks != null ? String(c.picks) : "", vegPicks: c.veg_picks != null ? String(c.veg_picks) : "", nonVegPicks: c.non_veg_picks != null ? String(c.non_veg_picks) : "", items: d.courseItems.filter(ci => ci.course_id === c.id).map(ci => ci.dish_id || `drink:${ci.drink_id}`), proteins: Object.fromEntries(d.courseItems.filter(ci => ci.course_id === c.id && ci.dish_id && ci.protein_options?.length).map(ci => [ci.dish_id, (ci.protein_options || []).join(", ")])), extraPrices: Object.fromEntries(d.courseItems.filter(ci => ci.course_id === c.id && Number(ci.extra_price_per_head) > 0).map(ci => [ci.dish_id || `drink:${ci.drink_id}`, String(ci.extra_price_per_head)])) })) : [{ key: crypto.randomUUID(), name: "Starters", picks: "", vegPicks: "", nonVegPicks: "", items: [] }];
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const [i, c] of courses.entries()) {
        if (!c.name) continue;
        const r = await (supabase.from("crm_package_courses" as any) as any).insert({ business_id: bid, package_id: pid, name: c.name, picks: c.picks !== "" ? Number(c.picks) : null, veg_picks: c.vegPicks !== "" ? Number(c.vegPicks) : null, non_veg_picks: c.nonVegPicks !== "" ? Number(c.nonVegPicks) : null, sort_order: i }).select().single(); if (r.error) throw r.error;
        if (c.items.length) { const ri = await (supabase.from("crm_package_course_items" as any) as any).insert(c.items.map(it => it.startsWith("drink:") ? { business_id: bid, course_id: r.data.id, drink_id: it.slice(6), extra_price_per_head: Number(c.extraPrices?.[it] || 0) } : { business_id: bid, course_id: r.data.id, dish_id: it, protein_options: (c.proteins?.[it] || "").split(",").map(x => x.trim()).filter(Boolean), extra_price_per_head: Number(c.extraPrices?.[it] || 0) })); if (ri.error) throw ri.error; }
      }
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_260px]">
      <div className="space-y-4">
        ...
        {courses.map((c, i) => <div key={c.key} className="space-y-3 rounded-md border p-4">
          <div className="flex items-end gap-2"><div className="flex-1 space-y-1.5"><Label>Course {i + 1} name *</Label><Input value={c.name} onChange={e => upd(c.key, { name: e.target.value })} /></div><Button size="icon" variant="ghost" onClick={() => setCourses(cs => cs.filter(x => x.key !== c.key))}><Trash2 className="h-4 w-4" /></Button></div>
          <div className={`grid gap-3 ${bev ? "sm:grid-cols-1" : "sm:grid-cols-3"}`}>
            <div className="space-y-1.5"><Label>{bev ? "Guest picks" : "Total picks (optional)"}</Label><Input type="number" min="0" placeholder="No limit" value={c.picks} onChange={e => upd(c.key, { picks: e.target.value })} /></div>
            {!bev && <div className="space-y-1.5"><Label>Vegetarian picks</Label><Input type="number" min="0" placeholder="No limit" value={c.vegPicks} onChange={e => upd(c.key, { vegPicks: e.target.value })} /></div>}
            {!bev && <div className="space-y-1.5"><Label>Non-vegetarian picks</Label><Input type="number" min="0" placeholder="No limit" value={c.nonVegPicks} onChange={e => upd(c.key, { nonVegPicks: e.target.value })} /></div>}
          </div>
          <div className="space-y-1.5">{c.items.map(it => { const p = pool.find(x => x.id === it); return <div key={it} className="grid gap-2 sm:grid-cols-[auto_minmax(10rem,1fr)_10rem]"><Badge variant="outline" className="h-8 justify-self-start gap-1">{p?.tag && <span className="text-primary">{p.tag}</span>}{p?.name || "Archived item"}<button type="button" onClick={() => upd(c.key, { items: c.items.filter(x => x !== it) })}><X className="h-3 w-3" /></button></Badge>{!bev ? <Input className="h-8 text-xs" placeholder="Protein choices (optional)" value={c.proteins?.[it] || ""} onChange={e => upd(c.key, { proteins: { ...(c.proteins || {}), [it]: e.target.value } })} /> : <span />}<Input className="h-8 text-xs" type="number" min="0" step="0.01" placeholder="$ extra/person" aria-label={`${p?.name || "Item"} extra price per person`} value={c.extraPrices?.[it] || ""} onChange={e => upd(c.key, { extraPrices: { ...(c.extraPrices || {}), [it]: e.target.value } })} /></div>; })}</div>
        ...
        <div className="flex flex-wrap items-center gap-2"><Button type="button" variant="outline" size="sm" onClick={() => setCourses(cs => [...cs, { key: crypto.randomUUID(), name: "", picks: "", vegPicks: "", nonVegPicks: "", items: [] }])}><Plus className="mr-1 h-4 w-4" />Add course</Button><span className="text-xs text-muted-foreground">or start from</span>{COURSE_PRESETS.map(p => <Button key={p} type="button" size="sm" variant="ghost" onClick={() => setCourses(cs => [...cs, { key: crypto.randomUUID(), name: p, picks: "", vegPicks: "", nonVegPicks: "", items: [] }])}>{p}</Button>)}</div>
      </div>
