  ...
  return <>
    <div className="space-y-4 rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="min-w-[220px] space-y-3 rounded-xl bg-secondary p-4 text-secondary-foreground shadow-md">
      ...
      <div className="overflow-hidden rounded-xl border border-primary/40">
        ...
        <div className="grid gap-3 bg-primary/5 p-4 text-sm sm:grid-cols-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.8fr_1fr]">
        <Box icon={UtensilsCrossed} title="Menu selection">
          ...
          {pkgs.length > 0 && <p className="font-semibold">{pkgs.map(p => p.item_name).join(" · ")}</p>}
          {Object.keys(courses).length > 0 && <p className="mb-2 mt-1 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Items</p>}
          <ul className="space-y-2">
            {(Object.entries(courses) as [string, any[]][]).map(([c, l]) => <li key={c}>
              <p className="flex items-center gap-2 text-sm font-semibold"><span className="h-1.5 w-1.5 rounded-full border border-primary" />{c}</p>
              <ul className="ml-6 mt-1 space-y-0.5">{l.map(x => <li key={x.id} className="flex items-center gap-2 text-sm"><span className="h-1 w-1 bg-muted-foreground" />{x.item_name}</li>)}</ul>
            </li>)}
          </ul>
        ...
        <div className="space-y-4">
          <Box icon={Star} title="Activities">
            {schedule.length ? <div className="space-y-3">{schedule.map((s, i) => <div key={i}>
              <div className="flex items-center gap-2"><span className="rounded-full bg-primary/20 px-2 py-0.5 text-xs font-medium text-primary">{s.time ? to12(s.time) : "—"}</span><span className="h-px flex-1 bg-primary/40" /><span className="h-1.5 w-1.5 rounded-full border border-primary" /></div>
              <p className="mt-1 text-sm">{s.label}{s.detail ? <span className="text-muted-foreground"> — {s.detail}</span> : null}</p>
      ...
      <Box icon={PenLine} title="Authorized signatures">
        <div className="mt-6 grid grid-cols-3 gap-6 text-center text-xs text-muted-foreground">
