...
export default function LeadsBoard({ kind }: { kind: "event" | "catering" }) {
  ...
  return <div className="space-y-5">
    ...
    <p className="text-xs text-muted-foreground">{shown.length} leads shown</p>
     <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{shown.map(l => <Card key={l.id} className="min-w-0"><CardContent className="min-w-0 space-y-3 p-5">
      <div className="flex items-start justify-between gap-2"><div><p className="font-serif text-xl">{l.full_name}</p><p className="text-xs text-muted-foreground">{kind === "event" ? `${prettyCrmValue(l.event_type)} · ` : ""}{prettyCrmValue(l.source)}</p></div><Badge variant={outcome(l) === "declined" ? "destructive" : outcome(l) === "confirmed" ? "default" : "outline"} className="capitalize">{outcome(l) === "new" ? "Lead" : outcome(l)}</Badge></div>
       <dl className="grid min-w-0 grid-cols-2 gap-2 text-sm"><div className="min-w-0"><dt className="text-xs text-muted-foreground">Email</dt><dd className="truncate" title={l.email || ""}>{l.email || "—"}</dd></div><div className="min-w-0"><dt className="text-xs text-muted-foreground">Phone</dt><dd className="truncate">{l.phone || "—"}</dd></div><div><dt className="text-xs text-muted-foreground">Guests</dt><dd>{l.estimated_guest_count || "—"}</dd></div><div><dt className="text-xs text-muted-foreground">Date</dt><dd>{l.preferred_dates?.[0] || "—"}</dd></div>{kind === "catering" && <div className="col-span-2 min-w-0"><dt className="text-xs text-muted-foreground">Service location</dt><dd className="break-words">{l.service_location || "—"}</dd></div>}{l.decline_reason && <div className="col-span-2 min-w-0"><dt className="text-xs text-muted-foreground">Declined because</dt><dd className="break-words">{prettyCrmValue(l.decline_reason)}</dd></div>}</dl>
      <div className="flex flex-wrap gap-2 pt-1">
        {outcome(l) === "new" && <>{kind === "catering" && <Button size="sm" onClick={() => nav(`/b/${businessCode}/catering/bookings/new?lead=${l.id}`)}>Confirm as catering job</Button>}{kind === "event" && <Button size="sm" onClick={() => setDetail(l)}>Continue workflow · {prettyCrmValue(l.status)}</Button>}<Button size="sm" variant="outline" onClick={() => setDeclining(l)}>Decline</Button></>}
        {outcome(l) === "declined" && <Button size="sm" variant="outline" onClick={() => reopen(l)}>Reopen</Button>}
        <Button size="sm" variant="ghost" onClick={() => { if (kind === "catering") { const b = crm.bookings.find(bk => bk.lead_id === l.id && bk.booking_kind === "catering"); nav(`/b/${businessCode}/catering/${b ? `bookings/${b.id}` : `leads/${l.id}`}`); return; } setDetail(l); }}>View details</Button><Button size="sm" variant="ghost" onClick={() => { setEditing(l); setFormOpen(true); }}>Edit</Button>
      </div>
