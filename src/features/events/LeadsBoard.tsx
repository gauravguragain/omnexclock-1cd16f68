...
export default function LeadsBoard({ kind }: { kind: "event" | "catering" }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-xs font-medium uppercase tracking-widest text-primary">Leads</p><h1 className="font-serif text-3xl font-semibold">{label}</h1><p className="text-sm text-muted-foreground">{kind === "event" ? "Enquiries about hosting an event — confirm one to turn it into an event." : "Enquiries for catering only — confirm one to turn it into a catering job."}</p></div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{shown.map(l => <Card key={l.id}><CardContent className="space-y-3 p-5">
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        {outcome(l) === "new" && <><Button size="sm" onClick={() => nav(`/b/${businessCode}/events/${kind === "event" ? "events" : "catering-bookings"}/new?lead=${l.id}`)}>Confirm as {kind === "event" ? "event" : "catering job"}</Button><Button size="sm" variant="outline" onClick={() => setDeclining(l)}>Decline</Button></>}
