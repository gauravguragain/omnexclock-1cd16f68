  ...
  return <div className="space-y-5">
    ...
    <p className="text-xs text-muted-foreground">{shown.length} leads shown</p>
     <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{shown.map(l => <Card key={l.id} className="min-w-0"><CardContent className="min-w-0 space-y-3 p-5">
      <div className="flex items-start justify-between gap-2"><div><p className="font-serif text-xl">{l.full_name}</p><p className="text-xs text-muted-foreground">{prettyCrmValue(l.event_type)} · {prettyCrmValue(l.source)}</p></div><Badge variant={outcome(l) === "declined" ? "destructive" : outcome(l) === "confirmed" ? "default" : "outline"} className="capitalize">{outcome(l) === "new" ? "Lead" : outcome(l)}</Badge></div>
