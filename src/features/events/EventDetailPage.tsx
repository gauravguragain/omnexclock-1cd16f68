...
export default function EventDetailPage({ kind }: { kind: "event" | "catering" }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-wrap gap-2">{lead && kind === "catering" && <><Button variant="outline" disabled={!rs} onClick={() => nav(`/b/${businessCode}/events/runsheet/${rs.id}`)}><FileText className="mr-2 h-4 w-4" />Preview run sheet</Button><Button variant="outline" onClick={async () => { if (!rs) { const { error } = await supabase.from("crm_runsheets").insert({ business_id: b.business_id, lead_id: b.lead_id, booking_id: b.id, event_order_number: b.event_order_number, adult_guests: b.adults ?? null, kids_guests: b.kids ?? null, status: "draft" } as any); if (error) return toast.error(error.message); await crm.reload?.(); toast.success("Run sheet created — press Send again"); return; } setSendOpen(true); }}><Mail className="mr-2 h-4 w-4" />{rs?.sent_at ? "Resend run sheet" : "Send run sheet"}</Button><Button onClick={() => { setWorkflowTab("timeline"); setWorkflow(true); }}><Pencil className="mr-2 h-4 w-4" />Edit</Button></>}{lead && kind !== "catering" && <><Button variant="outline" onClick={() => { if (rs?.sent_at) nav(`/b/${businessCode}/events/runsheet/${rs.id}`); else { setWorkflowTab("runsheet"); setWorkflow(true); } }}><FileText className="mr-2 h-4 w-4" />Run sheet</Button><Button onClick={() => { setWorkflowTab("timeline"); setWorkflow(true); }}><Pencil className="mr-2 h-4 w-4" />Edit</Button>{rs?.sent_at && <Button variant="outline" onClick={() => setSendOpen(true)}><Mail className="mr-2 h-4 w-4" />Resend Email</Button>}</>}</div>
    </div>
    ...
    {lead && rs && <SendRunsheetDialog open={sendOpen} onOpenChange={setSendOpen} rs={rs} lead={lead} booking={b} businessName={crm.business.name} mode={kind === "catering" && !rs.sent_at ? "issue" : "resend"} onIssue={async () => { const { data, error } = await supabase.from("crm_runsheets").update({ status: "issued", sent_at: new Date().toISOString() } as any).eq("id", rs.id).select().single(); if (error) { toast.error(error.message); return null; } return data; }} onSent={() => crm.reload?.()} />}
