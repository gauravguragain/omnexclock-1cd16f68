...
export default function MenuBookPicker({ packages, onAdd }: { packages: Pkg[]; onAdd: (pkg: Pkg, picks: { course: string; dish: any }[]) => void }) {
  ...
  return <div className="mt-2 space-y-3 rounded-md border border-border bg-muted/30 p-3">
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      ...
      <select className={sel} value={pkgId} disabled={!bookId} onChange={e => { setPkgId(e.target.value); setPicks({}); }}><option value="">Choose a package…</option>{packages.filter(p => p.book_id === bookId).map(p => <option key={p.id} value={p.id}>{p.name} (${Number(p.price_per_head)}/head{p.min_guests > 1 ? `, min ${p.min_guests}` : ""})</option>)}</select>
    </div>
