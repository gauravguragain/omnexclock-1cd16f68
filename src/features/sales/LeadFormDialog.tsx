...
export default function LeadFormDialog({ open, onOpenChange, businessId, options, lead, leads, onSaved, defaultKind = "event" }: { open: boolean; onOpenChange: (open:boolean)=>void; businessId:string; options:CrmOption[]; lead?:CrmLead|null; leads:CrmLead[]; onSaved:()=>void; defaultKind?: string }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle className="font-serif text-2xl">{lead ? "Edit lead" : "New lead"}</DialogTitle></DialogHeader><form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="space-y-1.5"><Label>Preferred date</Label><DateField name="preferred_date" defaultValue={lead?.preferred_dates?.[0] || ""} placeholder="Not set"/></div><div className="space-y-1.5"><Label>Guests</Label><Input name="guests" type="number" min="1" defaultValue={lead?.estimated_guest_count || ""}/></div>
    ...
    {!isCatering && <><div className="space-y-1.5"><Label>Budget from</Label><Input name="budget_min" type="number" min="0" defaultValue={lead?.budget_min || ""}/></div><div className="space-y-1.5"><Label>Budget to</Label><Input name="budget_max" type="number" min="0" defaultValue={lead?.budget_max || ""}/></div>
    <div className="space-y-1.5"><Label>Pipeline value</Label><Input name="estimated_value" type="number" min="0" defaultValue={lead?.estimated_value || ""}/></div><div className="space-y-1.5"><Label>Venue</Label><OptionSelect name="venue_space" options={list("venue_space")} defaultValue={lead?.venue_space || ""} emptyLabel="Not selected"/></div>
    <div className="space-y-1.5"><Label>Service location (catering)</Label><Input name="service_location" defaultValue={lead?.service_location || ""} placeholder="Address for catering jobs"/></div></>}
    {isCatering && <div className="space-y-1.5"><Label>Pipeline value</Label><Input name="estimated_value" type="number" min="0" defaultValue={lead?.estimated_value || ""}/></div>}
