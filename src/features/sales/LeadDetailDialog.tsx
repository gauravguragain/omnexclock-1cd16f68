...
export default function LeadDetailDialog({ lead, open, onOpenChange, options, interactions, inspections, tasks, menuItems, booking, businessName, onSaved }:{ lead:CrmLead|null; open:boolean; onOpenChange:(v:boolean)=>void; options:CrmOption[]; interactions:CrmInteraction[]; inspections:CrmInspection[]; tasks:CrmTask[]; menuItems:any[]; booking:any; businessName:string; onSaved:()=>void }) {
  ...
  const [liveStalls,setLiveStalls]=useState<{key:string;name:string;pricePerHead:number;flatPrice:number}[]>([]); const [stallDraft,setStallDraft]=useState({name:"",pricePerHead:"",flatPrice:""}); const [stallsRequired,setStallsRequired]=useState(false);
  ...
  const saveMenu=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);
    ...
    const pendingStall=stallsRequired&&pendingStallName&&!liveStalls.some(v=>v.name===pendingStallName)?[{key:`s${Date.now()}`,name:pendingStallName,pricePerHead:Number(stallDraft.pricePerHead||0),flatPrice:Number(stallDraft.flatPrice||0)}]:[];
    ...
    if(pendingStall.length){setLiveStalls(allStalls);setStallDraft({name:"",pricePerHead:"",flatPrice:""});}
