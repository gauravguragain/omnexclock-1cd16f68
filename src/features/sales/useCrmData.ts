...
export function useCrmData() {
  ...
  const [bookings, setBookings] = useState<any[]>([]); const [menuItems, setMenuItems] = useState<any[]>([]); const [loading, setLoading] = useState(true);

  const refresh = useCallback(async () => {
    if (!business?.id) return; setLoading(true);
    const [leadRes, optionRes, inspectionRes, taskRes, interactionRes, settingRes, bookingRes, menuRes] = await Promise.all([
    ...
    setBookings(bookingRes.data || []); setMenuItems(menuRes.data || []); setLoading(false);
  }, [business?.id]);

  useEffect(() => { void refresh(); }, [refresh]);
  return { business, leads, options, inspections, tasks, interactions, settings, bookings, menuItems, loading, refresh };
}